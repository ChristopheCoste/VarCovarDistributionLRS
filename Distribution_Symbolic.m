...
pgfF=[x(1)/2 + 1/2, x(1)/2 + x(2)/2] % pgf of reproduction, eq (d)

%% intermediary outputs

pgfS  = ones(1,s) +  (x- ones(1,s))*S; %pgf of survival; eqs 4 and (b)
vpa(pgfS,2)
...
F=expect_pgf(pgfF,x,s,s) % expectation of reproduction, eq (e)
VF=Var_pgf(pgfF,x,s,s)% variance covaraince of reproduction, eq (e)
% these two elements are not necessary to compute the pgf of LRS

%% pgf n1
pgfn0=x(1)*x(2) % eq (f)

pgfn1=subs(pgfn0,x,pgfF.*pgfS) % eq (h)
...
pgfR=(pgfF + ones(1,s)*D - ones(1,s))*D^(-1) %eqs 17 and (k)
...
ylabel('\emph{small} offspring','Interpreter','latex')
xlabel('\emph{large} offspring','Interpreter','latex')
