...
# Can be used for any structured population projection model
...
S <- matrix(c(0.3, 0, 0.2, 0.7), nrow = 2, byrow = TRUE) #eq (a)

# Reproduction probabilities (expectation of reproduction)
F <- matrix(c(0.5, 0.5, 0, 0.5), nrow = 2, byrow = TRUE) #eq (e)

# Variance of reproduction
VF <- matrix(c(0.25, 0.25, 0, -0.25, 0, -0.25,0 , 0.25), nrow = 4, byrow = TRUE) #eq (e)
...
VS <- J %*% S - kronecker(S, S) %*% J  # eq.4 and eq. (c)

# Projection of abundance vector over time
n0 <- c(1,1)
Vn1 <- (VS + VF) %*% n0
Vn1
matrix(Vn1, s, s) # unnumbered equation (after eq 5c)

# Expectation of LRS
R <- F %*% solve(diag(s) - S)  # eqs 9 and (i)
R
